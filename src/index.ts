...
let lastPrepareResponse: wallet.PrepareSendResponseType | null = null;
...
export function register(api: PluginApi) {
  ...
  api.registerTool({
    ...
    async execute(_id, params) {
      try {
        const mnemonic = params.mnemonic as string | undefined;
  ...
  api.registerTool({
    ...
    async execute(_id, params) {
      try {
        const method = (params.method as wallet.ReceiveMethod) || 'spark';
        const amountSats = params.amount_sats as number | undefined;
  ...
  api.registerTool({
    ...
    async execute(_id, params) {
      try {
        const paymentRequest = params.payment_request as string;
        const amountSats = params.amount_sats as number | undefined;
